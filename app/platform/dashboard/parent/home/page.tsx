...
const quickLinks = [
	{
		href: "/platform/dashboard/parent/children",
		labelKey: "children.title",
		descriptionKey: "children.manage",
		icon: Users,
	},
	{
		href: "/platform/dashboard/parent/wird",
		labelKey: "wird.title",
		descriptionKey: "wird.track",
		icon: ScrollText,
	},
	{
		href: "/platform/dashboard/parent/library",
		labelKey: "library.title",
		descriptionKey: "library.browse",
		icon: Library,
	},
	{
		href: "/platform/dashboard/parent/invoices",
		labelKey: "invoices.title",
		descriptionKey: "invoices.review",
		icon: Receipt,
	},
];
...
export default function ParentHomePage() {
	...
	const upcomingQuery = useQuery({
		queryKey: ["upcoming-sessions", studentId ?? "all"],
		queryFn: () => getMyUpcomingSessions({ studentId }),
		refetchInterval: 60_000,
	});

	const notificationsQuery = useQuery({
		queryKey: ["my-notifications", 1, 10],
	...
	return (
		<div className="flex flex-col gap-6">
			<PageHeader
				title={t("home.title_page")}
				description={
					profileQuery.isLoading
						? t("profile.loading_profile")
						: `Welcome back, ${profileQuery.data?.full_name ?? "parent"}!`
				}
			/>
			{profileQuery.isError ? (
				<ErrorBanner message={errorMessage(profileQuery.error)} />
			) : null}
			...
			<section className="flex flex-col gap-3">
				...
				{upcomingQuery.isSuccess && upcoming.length === 0 ? (
					<EmptyState
						...
						description={t("home.no_upcoming_parent_desc")}
					/>
				) : null}
				{upcomingQuery.isSuccess && upcoming.length > 0 ? (
							...
							>
								...
								<CardContent className="flex flex-col gap-2 text-sm">
									...
									<div className="mt-1 w-fit">
										<StatusBadge
											variant={
												session.minutes_until_start < 60
													? "success"
													: "secondary"
											}
										>
